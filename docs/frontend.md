...
- The landing-page search form now requires a ticker plus an explicit choice between the dashboard and valuation flows, and that valuation route now renders the first real calculator UI: a DCF workspace with live yfinance-backed company inputs, user-entered growth plus discount assumptions, and a local API proxy at `frontend/src/app/api/valuations/dcf/route.ts` for recalculation.
