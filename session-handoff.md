...
- Code or behavior added: Reworked the backend waterfall math so `Other Income / Cost` is now the residual bucket after direct `Operating Profit`, `Taxes`, and `Net Profits` are fetched, which keeps the displayed bridge numerically consistent even when Yahoo's pretax-related rows disagree. Added a backend regression test that locks the residual to `-50.3M` for the mismatch pattern the user reported.
- Infrastructure or harness changes: Refreshed the passing Playwright log at `artifacts/verification/dash-004-playwright.log`; the `dash-004` Playwright run passed on the first sandboxed attempt, and backend targeted pytest coverage now includes the residual-bucket regression.
- Documentation changes: `feature_list.json`, `progress.md`, and this handoff now point `dash-004` evidence at the residual-bucket fix commit while keeping `dash-005` as the next roadmap slice.
