...
- Code or behavior added: Refined the `dash-004` chart so it no longer forces a `$0` y-axis label, and restored the horizontal zero baseline as a dotted line with the same style and color as the other dotted chart lines.
- Infrastructure or harness changes: Refreshed the passing Playwright log at `artifacts/verification/dash-004-playwright.log`; the first test attempt was blocked by a stale local `next dev` process on PID `93136`, and the rerun passed after that process was terminated.
- Documentation changes: `feature_list.json`, `progress.md`, and this handoff now point `dash-004` evidence at the zero-baseline restoration commit while keeping `dash-005` as the next roadmap slice.
