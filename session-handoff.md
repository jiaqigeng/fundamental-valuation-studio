...
- Code or behavior added: Refined the `dash-004` chart so the restored horizontal zero baseline now renders above the bars, which keeps the dotted line visible without changing its style or the current axis-label behavior.
...
- Documentation changes: `feature_list.json`, `progress.md`, and this handoff now point `dash-004` evidence at the zero-baseline visibility fix while keeping `dash-005` as the next roadmap slice.
