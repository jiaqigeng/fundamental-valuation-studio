...
- What verification actually ran: `./init.sh`; `cd backend && .venv/Scripts/python.exe -m pytest tests/test_val_001a.py -q`; `cd frontend && npm.cmd run lint`; `cd frontend && npm.cmd run typecheck`; `cd frontend && cmd /c npx playwright test e2e/val-001b.spec.ts`

## Changed This Session

- Code or behavior added: Replaced the old revenue-and-margin DCF contract with a direct FCF DCF contract, removed the hardcoded valuation baseline path from the app, taught the Yahoo client to fetch current free cash flow plus debt, cash, shares, beta, risk-free rate, and price, rewired the frontend DCF panel around those fetched company inputs, and changed the user-editable assumptions to short-term FCF growth, terminal growth, equity risk premium, WACC or discount rate, and a 5-year or 10-year projection horizon.
