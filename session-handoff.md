...
- Code or behavior added: Polished the existing `dash-004` waterfall chart so the longer x-axis labels wrap cleanly across two lines and no longer overlap one another.
...
- Documentation changes: `feature_list.json`, `progress.md`, and this handoff now note that the passing `dash-004` waterfall also includes wrapped x-axis labels for readability.
